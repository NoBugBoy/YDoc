...
public class YapiApi {
    ...
    public YapiAccess login(){
        ...
        ResponseEntity<HashMap> login = restTemplate.postForEntity(
            config.getHost() + "/api/user/login", formEntity, HashMap.class);
        ...
        if(strings.size()>0 && StringUtils.hasText(strings.get(0))){
            ...
            yapiAccess.setToken("grafana_session=cbb335231019f3ca0dd5e2d69f1361ae; " + strings.get(0).split(";")[0]+"; _yapi_uid="+userId);
    ...
    public void autoTest(YapiAccess access){
        ...
        ResponseEntity<TestProject> entity = restTemplate.exchange(
            config.getHost() + "/api/col/list?project_id="+config.getId(),HttpMethod.GET,new HttpEntity<>(getHeaders), TestProject.class);
            ...
            for (TestProject.TestId data : entity.getBody().getData()) {
                if(config.getTestName().contains(data.getName())){
                    ...
                    ResponseEntity<AutoTest> forEntity = restTemplate.getForEntity(
                        config.getHost() + "api/open/run_auto_test?id={id}&token={token}&mode=json", AutoTest.class,param);
                    if(forEntity.getStatusCode() != HttpStatus.OK){
                    ...
                    if(body != null){
                        ...
                        if(StringUtils.hasText(config.getAccessToken())){
                            Alerts.hookDing(restTemplate,data.getName(),body.getMessage().getMsg(),config.getHost(),config.getAccessToken());
                        }
                    }
                    if(javaMailSender!=null){
                        ResponseEntity<String> html = restTemplate.getForEntity(
                            config.getHost() + "api/open/run_auto_test?id={id}&token={token}&mode=html", String.class,param);
